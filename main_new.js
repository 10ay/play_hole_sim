...
// New variables for the new modes
var program;
var locationOfMode;
var locationOfSpin;
var locationOfWormholePair;

var mode = 0;
var modeNames = ['Neutron Star', 'Wormhole'];
var modeLabel;
var wormholePair = { x: 0, y: 0, moved: false };
var shiftHeld = false;
...
function init(image) {
	...
	wormholePair.x = canvas.width * 0.78;
	wormholePair.y = canvas.height * 0.48;
	wormholePair.moved = false;

	$(document).mousemove(function(e) {
		if (shiftHeld && mode === 1) {
			wormholePair.x = e.pageX;
			wormholePair.y = -e.pageY + canvas.height;
			wormholePair.moved = true;
		} else {
			mouse.x = e.pageX;
	...
	*/
    ...
    locationOfMode = gl.getUniformLocation(program, "u_mode");
    locationOfSpin = gl.getUniformLocation(program, "u_spin");
    locationOfWormholePair = gl.getUniformLocation(program, "u_wormhole_pair");
	...
	*/
	...
	gl.uniform1f(locationOfMass, uMass);
...
function render() {
	...
	if(mode === 1 && wormholePair.moved == false){
		wormholePair.y = (-(originY/2) + Math.sin(currentTime * 0.5) * ((originY * 0.22))) + canvas.height;
		wormholePair.x = (originX * 0.72) + Math.sin(currentTime * 0.45) * -(originX * 0.28);
	}

    gl.uniform1i(locationOfMode, mode);
    gl.uniform1f(locationOfSpin, 2.0);
    gl.uniform2f(locationOfWormholePair, wormholePair.x, wormholePair.y);
...
window.addEventListener('keydown', function(event) {
    if (event.key === '1') mode = 0;
    if (event.key === '2') mode = 1;

    if (modeLabel) {
      modeLabel.textContent = 'Mode: ' + modeNames[mode];
    }
  });
